...
class RobustPhysicalConstrainedPostProcessing:
    ...
    def _normalize_channels(self, img_array):
        r_values = img_array[:, :, 0]  # 获取 R 通道
        g_values = img_array[:, :, 1]  # 获取 G 通道
        b_values = img_array[:, :, 2]  # 获取 B 通道

        # 检查 G 通道是否为零，如果为零则将其替换为一个非常小的常数值
        g_values = np.clip(g_values, 1e-8, None)  # 替换零值为非常小的数值

        # 归一化 R 和 B 通道
        normalized_r = r_values / g_values  # 归一化 R/G
        normalized_b = b_values / g_values  # 归一化 B/G
    ...
    def _cluster_rb_points(self, rb_points):
        ...
        # 将 illum_est 转换到rb空间（R/G和B/G），使用K-means聚类找到两主光源 l1 , l2 .
        kmeans = KMeans(n_clusters=2, random_state=666).fit(filtered_rb_points)  # filtered_rb_points为去噪后的色度
    ...
    def _identify_noise(self, rb_points, line_samples):
        ...
        """
        # 物理约束后处理第二步：噪声识别
        # 定义容忍范围：在两主光源连线间生成10个等距点Pk,每个点允许半径 σ= 0.2 的偏差。
        # 标记噪声： 由论文中的公式四得出：若像素Ip 到所有的 Pk 的距离 > σ ，则标记为噪声
        distances = np.sqrt(((rb_points[:, np.newaxis, :] - line_samples[np.newaxis, :, :]) ** 2).sum(axis=2))  # 计算距离
        min_distances = np.min(distances, axis=1)   # 标记噪声
...
if __name__ == "__main__":
    ...
    sigma = 0.2  # Adjust sigma as needed， 每个点允许半径 σ= 0.2 的偏差
