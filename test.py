...
USING_POST_PROCESSING = True  # 控制是否启用物理约束后处理
if USING_POST_PROCESSING:
    robust_post_processing = RobustPhysicalConstrainedPostProcessing()
# ----------------------------------------------------------------------
MODEL_PATH = f"pretrained_models/ours_galaxy"
TEST_DATA_PATH = 'LSMI_dataset/galaxy_512'
...
def test_net(net, data_dir):
    ...
    for batch in test_loader:
        ...
        with torch.no_grad():
            ...
            # 计算初始光照估计图
            # 模型预测的 pred_uv 需要转换回 RGB 空间进行可视化或评估
            pred_rgb = apply_wb(input_rgb, pred_uv.detach(), pred_type='uv')  # 从预测UV生成白平衡图像 ，  input_rgb是原始图像

            # 计算光照估计，对应论文中的 初始光照估计图
            pred_illu = input_rgb / (pred_rgb + EPS)    # 光照估计 = 原始输入 / 白平衡结果  ， EPS避免除零

            pred_illu[:, 1, :, :] = 1.   # 固定G通道为1   ，  初始光照估计图，值≠1的部分表示光照色偏。
