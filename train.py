...
from model import Transformer_UNet
...
def train_net(net,
    ...
    logging.info(f'''Start training:
            ...
            Div factor:             {div_factor}
            Final div factor:       {final_div_factor}
            
            === Network Structure Optimization ===
            Local-Global Attention: {use_local_global_attention}
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # 损失计算
            pixel_loss = criterion(pred_uv, gd_uv)
            total_loss = pixel_loss + angular_w * mae_illu
            loss_dict = {
                'pixel_loss': pixel_loss.item(),
                'angular_loss': mae_illu.item(),
            ...
            log_msg = (
                ...
                f"UV_loss:{loss_dict['pixel_loss']:.4f} "
                f"Ang_loss:{loss_dict['angular_loss']:.4f} "
        ...
        if epoch % validation_frequency == 0:
            ...
            logging.info("The current metrics are:")
            ...
            if val_loss.avg < best_val_error:
                ...
                data_tag = os.path.basename(args.trdir).replace("_", "").lower()
                model_filename = f'model_best_{data_tag}.pth'
                logging.info(f"Saving the best model as {model_filename}...\n")
                torch.save(net.state_dict(), os.path.join(logdir, model_filename))
            else:
            ...
            if validation_frequency * epochs_without_improvement >= stopping_patience:
                logging.info("Training stopped! Best model has been saved!")
                logging.info(
...
# val_net and get_data_loaders 保持不变…
    ...
    with tqdm(total=len(val_loader), desc="Validation Processing",
        ...
        for batch in val_loader:
            ...
            with torch.no_grad():
                ...
                evaluator.add_error(mae_illu) # 聚合评估指标（通过Evaluator）
...
def get_data_loaders(data_dir, batch_size):
    # 训练集：启用光照增强（RandomColor）和随机裁剪（aug_crop）
    ...
    # 验证集：仅调整大小（val_resize），batch_size=1
...
if __name__ == '__main__':
    ...
    log_dir = f"debug_2000_test_nikon/"
    ...
    # 初始化Transformer-UNet模型
    net = Transformer_UNet(
        img_ch=args.img_ch,
        output_ch=args.output_ch,
        d_model=args.d_model,
        nhead=args.nhead,
        num_layers=args.num_layers,
        use_local_global_attention=args.use_local_global_attention
    )
    net.to(DEVICE)
    ...
    train_net(net=net,
              ...
              angular_weight=args.angular_loss_weight,
              use_local_global_attention=args.use_local_global_attention)
